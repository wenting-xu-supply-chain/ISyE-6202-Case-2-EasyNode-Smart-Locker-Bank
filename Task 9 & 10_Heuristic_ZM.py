...
max_demand_per_period = Daily_demand.groupby(["year","period"])['final_daily_demand'].max()
...
for (year, period), max_demand in max_demand_per_period.items():
    ...
    # locker 分组
    S_lockers = [locker for locker  in Locker_key if locker .startswith("S")]
    M_lockers = [locker  for locker  in Locker_key if locker .startswith("M")]
    L_lockers = [locker  for locker  in Locker_key if locker .startswith("L")]
    ...
    row = {
        ...
        "S": final_lockers[[Locker_key.index(locker) for locker in S_lockers]].sum(),
        "M": final_lockers[[Locker_key.index(locker) for locker in M_lockers]].sum(),
        "L": final_lockers[[Locker_key.index(locker) for locker in L_lockers]].sum(),
    }
