...
# Mean Demand in period
Mean_packages_periods = []
num_periods = int(math.ceil(len(Daily_demand_by_size) / 28))
Safety_factor = 1 + norm.ppf(0.995) * 0.25
for p in range(num_periods):
    start_idx = p * 28
    ...
    if not period_data.empty:
        mean_s = np.mean(period_data["S"]) * Safety_factor
        mean_m = np.mean(period_data["M"]) * Safety_factor
        mean_l = np.mean(period_data["L"]) * Safety_factor
        
        Mean_packages_periods.append({
            "period": p + 1,
            "S": math.ceil(mean_s),
            "M": math.ceil(mean_m),
            "L": math.ceil(mean_l)
...
Mean_packages_df = pd.DataFrame(Mean_packages_periods)
Mean_packages_df.to_csv("periodic_mean_demand.csv", index=False)