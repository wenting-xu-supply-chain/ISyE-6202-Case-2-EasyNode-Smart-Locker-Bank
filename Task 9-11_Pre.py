...
num_periods = int(math.ceil(len(Peak_daily_demand_by_size) / 28))
Safety_factor = 1 + norm.ppf(0.995) * 0.25
for p in range(num_periods):
    start_idx = p * 28
    end_idx = min((p + 1) * 28, len(Peak_daily_demand_by_size))
    
    period_data = Peak_daily_demand_by_size.iloc[start_idx:end_idx]
